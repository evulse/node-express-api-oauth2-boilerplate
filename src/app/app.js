...
app.del('/test/:id.:format', function (req, res) {
  res.send(204);
});

app.put('/test.:format', function (req, res) {
...
app.get('/test/:id.:format?', function (req, res) {
  if (req.params.id === '1') {
    res.json(200, {
      "name": "Hello",
      "price": 50.00
    ...
    res.json(404, {
      ...
      "error_uri": "http://api.carted.com/errors"
    });
  }
});

app.post('/test.:format?', function (req, res) {
  ...
  } else { // for now, assume else if posted with good date
    res.json(201, {
      "test": {
        "name": "Hello",
        "price": 50.00
